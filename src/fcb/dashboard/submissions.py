...
def _detail_block(row: dict, submission_id: int) -> str:
    ...
    def action_button(target_status: str, label: str) -> str:
        if row["status"] == target_status:
            return f'<span class="btn secondary" style="opacity:0.5;">{escape(label)}</span>'
        return (
...
async def list_submissions_page(
    ...
    if not rows:
        ...
        for r in rows:
            event_link = (
                f'<a href="/events/{r["event_id"]}">{escape(r["event_name"] or "?")}</a>'
                if r["event_id"]
...
<div class="card">
  <div style="display:flex;gap:0.75rem;flex-wrap:wrap;align-items:center;margin-bottom:1rem;">
    {status_filters}
    <form method="get" action="/submissions" style="display:inline-flex;gap:0.5rem;align-items:center;margin-left:auto;">
...
async def submission_detail_page(
    ...
    event_link = (
        ...
        if row["event_id"]
        else '<span class="muted">no event</span>'
    )
...
<div class="card">
  ...
  <pre style="background:var(--panel-2);padding:1rem;border-radius:6px;overflow-x:auto;
              white-space:pre-wrap;">{escape(stats_pretty) or '<span class="muted">(none)</span>'}</pre>
</div>
...
"""
    return HTMLResponse(
        page(title=f"Submission #{submission_id}", body=body, admin=admin, active="submissions")
    )
...
async def flip_submission_status(
    ...
    dao.update_submission_status(
        submission_id, status=status, reviewed_by=admin["user_id"]
    )
    logger.info(
        f"admin {admin['user_id']} flipped submission #{submission_id} to {status!r}"
    )
    dao.record_event(
        level="INFO",
